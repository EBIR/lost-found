	...
	<?php 
		...
		if(isset($_GET['id'])) {
			...
			if($result->num_rows > 0) {
				...
				echo "<h5>".date( 'F jS', strtotime($publication['fecha']))." at ".date( 'g:i A', strtotime($publication['fecha']))."</h5>";
				echo "<b><h3>".$publication['titulo']."</h3></b>";
				echo "<p>".$publication['contenido']."</p>";
				echo "<b>Por: </b>";?><b style="color:#4CAF50;"> <?php echo $user['nombre']?> </b><?php
				?>

				<form action= "procesoComentario.php" method="post">
					<textarea name="comentario" class="registro" placeholder="Comentario..." style="color:black; text"></textarea>
				...
				if($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
						...
						?><b style="color:#4CAF50;"> <?php echo $user2['nombre']?> </b><?php
